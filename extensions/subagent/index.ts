/**
 * Subagents — spawn background subagents as in-process pi sessions
 * (createAgentSession via the pi SDK), managed by an Effect core.
 *
 * Tools (for the parent LLM):
 * - subagent_spawn: fire-and-forget spawn (prompt, name, agent, working_dir,
 ...
 * (live transcript, steer mid-run, abort). `/btw` asks a one-off side
 * question.
 *
 * Named agents (this repo's convention): .md files in ~/.pi/agent/agents and
 * .pi/agents (project, trusted) define a system prompt, optional tool
 * allowlist, and optional model. Pass the name as `agent` to subagent_spawn.
 *
 * Architecture: Effect v4 generators throughout (backend -> manager ->
 * runtime); this file is the async boundary where tool handlers run effects
 * against one shared ManagedRuntime. Also emits event-bus notifications
 * (subagent/running, subagent/progress, subagent/settled) consumed by the
 * widget-card extension.
 */
...
/**
 * `subagents.defaultModel` from the global settings.json, overridden by the
 * project's .pi/settings.json. Undefined when neither sets it.
 */
function readSubagentsDefaultModel(cwd: string): string | undefined {
	const candidates = [
	...
	for (const file of candidates) {
		try {
			const raw = JSON.parse(fs.readFileSync(file, "utf8")) as {
				subagents?: { defaultModel?: unknown };
			};
			const model = raw.subagents?.defaultModel;
...
/** Resolve a named agent for a spawn; project agents require project trust. */
function resolveAgent(
	...
	const agent: AgentConfig | undefined = agents.find(
		(a) => a.name === agentName,
	);
	if (!agent) {
		const known = agents.map((a) => a.name).join(", ") || "none";
		throw new Error(`Unknown agent "${agentName}". Available: ${known}.`);
	}
	if (agent.source === "project" && !projectTrusted) {
		throw new Error(
			`Agent "${agentName}" comes from project-local .pi/agents and this project is not trusted. Trust the project or omit 'agent'.`,
		);
	}
	return {
		name: agent.name,
...
/** Stable, user-agent portion of the spawn tool description (session-scoped). */
function spawnToolDescription(): string {
	let agentsText = "none available";
	try {
		...
		// Fall back to the generic hint below.
	}
	return (
		`${SUBAGENT_SPAWN_TOOL_DESCRIPTION}\n` +
		`- ${agentsText}\n` +
		`Project-local agents (.pi/agents) are also available when the project is trusted.`
	);
}
...
export default function (pi: ExtensionAPI) {
	...
	/**
	 * Event-bus notifications for the widget-card. `subagent/running` fires
	 * only when the running count changes; `subagent/progress` is a throttled
	 * one-line preview of the most active subagent.
	 */
	const notifyWidget = (manager: SubagentManagerShape) => {
		const subs = manager.view.list().filter(isModelVisible);
		const running = subs.filter((snap) => snap.status === "running");
		if (running.length !== lastRunning) {
			lastRunning = running.length;
			pi.events.emit("subagent/running", {
		...
		if (running.length > 0) {
			...
			if (now - lastProgressEmit >= PROGRESS_EMIT_INTERVAL_MS) {
				...
				// Most active = last settled/created? Simplest stable pick:
				// the first running subagent with the most turns.
				const active = [...running].sort(
					(a, b) => b.turns - a.turns || a.createdAt - b.createdAt,
				)[0];
				pi.events.emit("subagent/progress", {
					agent: active.title,
					turns: active.turns,
					output: latestText(active).replace(/\s+/g, " ").trim(),
				});
			}
		}
	};
	...
	const onSettled = (snap: SubagentSnapshot, consumed: boolean) => {
		if (sessionContext) {
			try {
				pi.events.emit("subagent/settled", {
					id: snap.id,
					title: snap.title,
					status: snap.status,
				});
			} catch {
				// Widget plumbing is best-effort.
			}
		}
	...
	pi.on("session_shutdown", async () => {
		...
		// subagent scopes (and, later, their child sessions).
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			const agentName = params.agent?.trim() || undefined;
			...
			const modelHint =
				params.model ?? agent?.model ?? readSubagentsDefaultModel(ctx.cwd);
			const title =
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			...
			for (const id of ids) {
				...
				const headerBytes =
					Buffer.byteLength(section, "utf8") + 2;
