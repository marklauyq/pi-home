...
/**
 * Shared reasoning-effort scale (pi's thinking levels). Omitted = inherit
...
/**
 * Optional named-agent spec resolved by the tool layer from agent .md files
 * (frontmatter tools/model + body system prompt). Injected into the child's
 * task prompt; the tools allowlist restricts the child's tool set.
 */
export interface AgentSpec {
  readonly name: string;
  readonly systemPrompt: string;
  readonly tools?: ReadonlyArray<string>;
  /** Frontmatter model hint ("provider/model-id" or bare id). */
...
export interface SpawnTask {
  ...
  readonly cwd: string;
  /** Model hint: "provider/model-id" or bare model id. Omitted = inherit. */
  readonly model?: string;
  /** Shared effort scale (pi thinking levels). */
  readonly reasoningEffort?: ReasoningEffort;
  /** Named agent (scout/planner/reviewer/worker or any .pi/agents/*.md). */
...
export interface SubagentMeta {
  ...
  /** Display label, e.g. "llama-server/qwen35b-nvfp4". */
  readonly modelLabel?: string;
  /** Context window capacity for utilization display, when known. */
  readonly contextWindow?: number;
  /** pi session file (visible in /resume). */
