/**
 * The unified backend interface: one `SubagentBackend` per agent runtime, all
 * producing the same `SubagentSession` shape.
 *
 * This installation runs the pi backend only (in-process `createAgentSession()`
 * via the pi SDK). The interface keeps the backend swappable.
 */
