 ...
 * (subagent_* would recurse; bg_* terminals die with the child anyway.) */
  ...
  Effect.gen(function* () {
    ...
    const session = yield* Effect.tryPromise({
      try: async () => {
        ...
        // Named agents with a tools allowlist get exactly those tools; everyone
        // else gets the default set minus the recursion denylist.
        const toolOptions = task.agent?.tools
    ...
    emit({ _tag: "MetaChanged", meta: currentMeta() });
    // Named agents get their system prompt prepended to the task prompt: the
    // in-process session's system prompt is built from cwd resources and is
    // not extendable, so the agent instructions travel as user-visible text.
    const effectivePrompt = task.agent?.systemPrompt.trim()
      ? [
          `You are "${task.agent.name}", a specialized subagent. Your operating instructions:\n\n${task.agent.systemPrompt.trim()}\n\n---\n\nTask:`,
          task.prompt,
        ].join("\n")
      : task.prompt;
    startRun(effectivePrompt);
